...
fetch(`http://localhost:3000/api/cameras/${location.search.substring(1)}`)
    ...
    function cart (item) {
        // créer un tableau qui sera alimenté par les éléments récupéres dans localStorage
        let cartElement = []
        let sessionElement = {
            image: item.imageUrl,
            name: item.name,
        ...
        // Créer un tableau dans localStorage si vide avec élément
        if (cart === null) {
            cartElement.push(sessionElement);
            localStorage.setItem('panier', JSON.stringify(cartElement));
        } 
        // Ou récupère le tableau du localStorage et ajoute le nouveau produit
        else { 
